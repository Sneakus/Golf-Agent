...
| No mechanical jargon | Banned-term list checked across every text field. Plain replacements are applied after the answer is written, not in the main prompt | Same |
...
| 25 Sep 2026 | Last paid batch. Seven entries, strict tools, temperature 0. An answer that still fails after repair is not shown; the golfer gets the low-confidence question about where the ball started and which way it curved | 8/8 | 0 | 0 shown; query 13 clarified | 7/59 | | | about $0.47 at batch half price | 59 of 59 diagnosed entries matched. Wilson 95% interval for 59 of 59, and for hit@7, is about 0.939 to 1.000; a bootstrap interval collapses to a point when every item passes. Manual review: 46 fully correct, 11 flawed, 2 wrong. Query 38 regressed: the golfer said the break never took, and the answer says the putt was too slow. Query 13 failed validation because the golfer never said where the ball started, and now gets that clarifying question. Paid iteration stops here. Each round fixes its targets and changes one or two others, which is the point of diminishing returns. The eval set is saturated on diagnosis, so further improvement needs new queries not used to tune the system. |
| 25 Sep 2026 | Wording rewrites. Plain wording is checked after the answer is written. A failed wording rewrite is shown with the original wording. The corpus fingerprint ignores line endings | 8/8 | 0 | 0 | | | | $0.06 | 58 of 58 shown answers correct, 58 of 59 overall. Query 13 clarified. Query 55 shown with its original wording. 0 failures. About a third of answers need the extra rewrite, so the next step is to put the plain-word list in the main prompt. Entries tied at the edge of the top 7 can occasionally swap between runs. |
